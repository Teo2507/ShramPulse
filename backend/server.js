...
app.post('/api/migration/update',async(req,res)=>{try{const b=req.body||{};const {data:worker}=await supabase.from('workers').select('*').eq('id',b.workerId).single();if(!worker)return res.status(404).json({message:'Worker not found.'});const loc=findLocation(b.city||b.district,b.state)||{state:clean(b.state),district:clean(b.district),city:clean(b.city)};if(!loc.state||!loc.city)return res.status(400).json({message:'State and city are required.'});const occupation=detectOccupation(b.occupation||'',worker.occupation||'Other');const now=new Date().toISOString();const same=worker.current_state===loc.state&&worker.current_city===loc.city;const event = {
  id: `ME-${crypto.randomBytes(6).toString('hex').toUpperCase()}`,
  worker_id: worker.id,
  worker_name: worker.name,
  from_state: worker.current_state,
  from_city: worker.current_city,
  to_state: loc.state,
  to_city: loc.city,
  occupation,
